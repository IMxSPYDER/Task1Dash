...
const CalendarDay = styled(Box)(({ theme, hoverHeight }) => ({
  minHeight: hoverHeight ? `${hoverHeight}px` : "120px",
  height: hoverHeight ? `${hoverHeight}px` : "max-content",
...
const SocialMediaCalendar = () => {
  ...
  const renderCalendarDay = ({ date, postsForDay, rowIndex }) => {
    const dateString = date ? date.toLocaleDateString("en-US") : "";
    return (
      <CalendarDay
        hoverHeight={hoveredRowIndex === rowIndex ? hoverHeight : null}
